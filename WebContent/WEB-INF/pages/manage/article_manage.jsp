...
$(function(){
	//选择类别跳转 
      $("#articleTypeSelect").change(function(){
	    var typeId=$(this).val();
	    window.location='<%=path%>/mred/article/browse_article?article.articleType.id='+typeId;
	  });
	//全选
          ...
          for (x in ids)
				{
				 ...
				 //alert(param);
	             window.location='<%=path%>/mred/article/deleteBatch_article?'+param;
	    });
          ...
          for (x in ids)
				{
				 ...
				 //alert(param);
	             window.location='<%=path%>/mred/article/auditBatch_article?'+param;
	    });
          ...
          for (x in ids)
				{
				 ...
				 //alert(param);
	             window.location='<%=path%>/mred/article/pubBatch_article?'+param;
	    });
...
<table width="800" height="30" border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
    ...
    <s:select id="articleTypeSelect" cssClass="redInput" name="article.articleType.id" list="articleType" listKey="id" listValue="name" headerKey="0" headerValue="请选择类别"></s:select></td>
...
<table width="800" border="0" align="center" cellpadding="0" cellspacing="0" id="artList">
  <tr>
	...
	<s:iterator value="articleList" status="state"> 
		<s:form method="post"  name="/mred/article">
	  <tr
      <s:if test="#state.odd">
      class="bg1"
      </s:if>
      >
		<td>
		 <s:token name="redmenagearticletoken"></s:token>
		 <input type="checkbox" class="checkboxfor"  value="<s:property value="id"/>"/>
         <input type="hidden"  name="article.id" value="<s:property value="id"/>"/>		</td>
		<td align="center" valign="middle"><s:property value="#state.index+1"/></td>
		<td align="left" valign="middle"><b>[ 
	    <s:property value="articleType.name"/>]</b></td>
		<td align="left" valign="middle">
		<a target="_blank" href="html/article/a<s:property value="articleType.id"/><s:property value="id"/>.html">
        <s:property value="%{@com.red.util.HtmlRegexpUtil@getDesc(title,24)}"/>
        </a>		</td>
		<td align="center" valign="middle"><s:date name="dates" format="yyyy-MM-dd"/></td>
		<td>
		<s:if test="%{ispass}">已审核</s:if>
		<s:else>
		<s:submit value="审核" action="audit_article" cssClass="redButton"/>
		</s:else>
		</td>
		<td><s:submit value="修改" action="update_article" cssClass="redButton"/></td>
		<td><s:submit value="删除" action="delete_article" cssClass="redButton"/></td>
	  </tr>
	  </s:form>
	</s:iterator>
  <tr>
    ...
    <pg:pager
    url="mred/article/browse_article"
    items="${totalCount}"
    index="center"
    maxPageItems="20"
    maxIndexPages="10"
    ...
    scope="page">
    <pg:param name="article.articleType.id" value="${article.articleType.id}"/>
    &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			<pg:first>
